...
//    fr_config.SINGLE_SLOT_ENABLED = 0;
    ...
    ret = tsflexray_set_controller_frametrigger(handle, 1, &fr_config, FrameLengthArray1, 3, fr_trigger1, 3, 3000);
    std::cout << "set controller result: " << ret << std::endl;
...
//    ret = tsflexray_register_event_flexray_whandle(handle1, &flexray_rx_event);
    ...
    std::cout << "flexray start net, result: " << tsflexray_start_net(handle, 1, 3000) << std::endl;
...
//    }
    ...
    for (int i = 0; i < 10; i++) {
        ...
        FlexRayFrameConfig frFrame[2000];
        int32_t size = 2000;
        int32_t count_size = 0;

        int fifo_ret = tsfifo_receive_flexray_msgs(handle, frFrame, &size, 0, 0);
        ...
        fifo_ret = tsfifo_receive_flexray_msgs(handle, frFrame, &size, 1, 0);
        std::cout << "FIFO ret is: " << fifo_ret << std::endl;
        for (int j = 0; j < size; j++) {
            if (frFrame[j].FSlotId > 60 && frFrame[j].FSlotId < 129 && frFrame[j].FFrameType == 0) {
                printf("FIFO Received dynamic frames: chn:%d slotID:%d cycle:%d dataLen:%d\n",
    ...
    std::cout << "stop net chn0: " << tsflexray_stop_net(handle, 0, 1000) << std::endl;
    std::cout << "stop net chn1: " << tsflexray_stop_net(handle, 1, 1000) << std::endl;
#ifdef _WIN32
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
#else
    sleep(1);
