 ...
 */
#ifndef __TSDEV_API_TAC_H
#define __TSDEV_API_TAC_H

#include "tsdev_types.h"
#include "tsdev_api_tac_types.h"

#ifdef __cplusplus
extern "C"
{
#endif
    TSDEV_API tsdev_api_tac_run_once_image(tsdev_handle_t handle);
    TSDEV_API tsdev_api_tac_run_image(tsdev_handle_t handle, tsdev_tac_exe_type_t tac_type);
    TSDEV_API tsdev_api_tac_stop_image(tsdev_handle_t handle, tsdev_tac_exe_type_t tac_type);
    TSDEV_API tsdev_api_tac_delete_all_image(tsdev_handle_t handle);
    TSDEV_API tsdev_api_tac_delete_image_by_type(tsdev_handle_t handle, tsdev_tac_exe_type_t type);

    // in json format
    TSDEV_API tsdev_api_tac_read_hw_info(tsdev_handle_t handle,
                                         void* usr_arg,
                                         tsdev_tac_cmd_event_t cmd_evt,
                                         tsdev_tac_data_event_t data_evt,
                                         uint32_t timeout_ms);

    TSDEV_API tsdev_api_tac_download_image(tsdev_handle_t handle,
                                           tsdev_tac_config_t* conf_p,
                                           void* usr_arg,                                           
                                           tsdev_tac_cmd_event_t cmd_evt,
                                           tsdev_tac_data_event_t data_evt,
                                           uint32_t timeout_ms);
